...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_numbers_divisible_by_3
    ...
    remaining = []
    numbers.each do |num|
      remaining << num unless (num % 3).zero?
    end
  ...
  def test_remove_dinosaurs
    ...
    notasaurus = []
    animals.each { |animal| notasaurus << animal unless animal.end_with?('saurus') }
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each { |animal| remaining << animal unless animal.start_with?('a','e', 'i', 'o', 'u')}
